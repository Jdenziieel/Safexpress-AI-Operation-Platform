"""
Lambda Function: quota-admin-restore
Restore a soft-deleted (deactivated) user (Admin only)
    ...
    try:
        ...
        if user_data.get('is_active', True):
            return {
                'statusCode': 400,
                'headers': cors_headers,
                'body': json.dumps({'error': f'User {user_id} is already active'})
            }
        ...
        quotas_table.update_item(
            Key={'user_id': user_id},
            UpdateExpression='SET is_active = :active, deactivated_at = :null, updated_at = :now',
        ...
        # Log admin action
        log_admin_action(admin_id, admin_name, 'restore_user', user_id, 
                        user_data.get('fullname', ''),
                        {'tier': user_data.get('tier', 'free')})
        
        print(f"Admin {admin_id} restored user {user_id}")
        ...
        return {
            'statusCode': 500,
            'headers': cors_headers,
            'body': json.dumps({'error': f'Failed to restore user: {str(e)}'})
        }
