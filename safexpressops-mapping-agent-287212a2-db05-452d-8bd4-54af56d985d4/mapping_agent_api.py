...
def _detect_sections(raw_values, min_section_rows=0):
    ...
    FOLLOW-UP / DEBT — see Documents/tickets/TICKET-001-section-detection-lambda-layer.md
        This function is a duplicate of
        ``dynamic mapping agent/lambda_function.py::_detect_sections_local``.
        Both copies must stay in lock-step (we've already been bitten twice:
        the min_section_rows=2 → 0 drift, and the TC-L03 Inbound Matrix
        single-titled-section write failure on 2026-05-11). Planned follow-up:
        extract into a shared ``safexpressops_common`` module uploaded as a
        Lambda layer. Until then, any behavior change here MUST be applied to
        the dynamic mapping agent copy as well.
    """
    sections = []
    i = 0
    ...
    # Headerless-top synthesis:
    # If we found exactly one explicit titled section, probe the rows above it
    # for an untitled top block whose first non-empty row is a plausible header.
    # This catches layouts like:
    #   row 1: Date | Dispatched | Cases   (no title row)
    #   row 2+: data
    #   ...
    #   row N: Inbound Metrics             (title)
    #   row N+1: Date | Trucks | Pallets   (header)
    # where the legacy detector only found the lower titled section and then
    # suppressed it via the >=2 filter.
    if len(sections) == 1:
        first_titled = sections[0]
        first_title_row = int(first_titled.get('title_row', 0))

        header_row_idx = None
        for probe_idx in range(0, max(0, first_title_row)):
            probe_row = raw_values[probe_idx]
            probe_non_empty = [str(c).strip() for c in probe_row if c and str(c).strip()]
            if len(probe_non_empty) >= 2 and any(_looks_like_header_value(v) for v in probe_non_empty):
                header_row_idx = probe_idx
                break

        if header_row_idx is not None and header_row_idx < first_title_row:
            data_start = header_row_idx + 1
            data_end = first_title_row

            # Ignore blank spacer rows between the top block and the first
            # titled section.
            while data_end > data_start:
                spacer_probe = raw_values[data_end - 1]
                if any(c and str(c).strip() for c in spacer_probe):
                    break
                data_end -= 1

            has_data_rows = any(
                any(c and str(c).strip() for c in r)
                for r in raw_values[data_start:data_end]
            )
            if has_data_rows and (data_end - data_start) >= min_section_rows:
                header_row = raw_values[header_row_idx]
                sec_headers = [str(c).strip() if c is not None else '' for c in header_row]
                sec_header_index = {h: idx for idx, h in enumerate(sec_headers) if h}
                sections.insert(0, {
                    'title': None,              # implicit/headerless section
                    'title_row': header_row_idx, # section starts at header row
...
def structure_target_data(raw_values: list, sheet_name: str = '', sample_size: int = 5) -> dict:
    try:
        ...
        if sections:
            ...
            # Single-section title layout (TC-L03 "Inbound Matrix" case,
            # 2026-05). _detect_sections suppresses results below 2 sections
            # so a target tab with row 0 = "Inbound Metrics" (1-cell title
            # banner) and row 1 = "Date | Trucks | Pallets" (real headers)
            # falls through here. Without this branch _detect_multi_row_headers
            # then mis-classifies row 0 as a group banner and produces
            # composite headers ("Inbound Metrics > Date", ...) that never
            # overlap a source whose headers are simply Date/Trucks/Pallets.
            # The orchestrator then bails with "no useful overlap" even
            # though the data shape clearly matches.
            #
            # Trigger criteria: _detect_sections_single_pass returns exactly
            # one TITLED section (PATH 1 of that helper — single non-empty
            # cell in title row + >=2 alphabetic header cells). Headerless
            # sections (PATH 2) are intentionally excluded; those are
            # already handled by the existing fallthrough path because
            # row 0 IS the header row.
            single_pass = _detect_sections_single_pass(raw_values, min_section_rows=0)
            titled_single = [s for s in single_pass if s.get('title')]
            if len(titled_single) == 1:
                first_sec = titled_single[0]
                sections = [first_sec]
                headers = [str(h).strip() for h in first_sec['headers']]
                # data_start already accounts for both title row + header row
                # (set to title_row + 2 in _detect_sections_single_pass) so
                # using it as header_row_count keeps col_samples slicing
                # consistent with the multi-section branch above.
                header_row_count = first_sec.get('data_start', 2)
                composite_to_col_index = {}
                print(
                    f"   Detected single-section layout: title="
                    f"{first_sec.get('title')!r}, headers={headers[:5]}"
                )
            else:
...
def find_identifier(target_schema: dict, source_schema: dict, source_section_title: str = None) -> dict:
    ...
    try:
        ...
        if not unmatched_sources:
            ...
            # Fix 1b: empty-target downgrade. When _infer_strategy_local
            # returns row_per_entity / composite_key but the chosen
            # anchor's TARGET column has < 2 non-empty values AND no
            # other mapped column shows entity overlap, the target is
            # effectively empty for this anchor. Routing such writes
            # through row_per_entity yields zero matches plus orphan
            # handling; routing them through 'append' lets the multi-
            # sheet aggregate-append branch (or the single-sheet append
            # path) accumulate cleanly. Without this, SKU IDENTIFIER ->
            # empty APPEND tab regresses from 'append + 6 new rows' to
            # 'row_per_entity + 0 matched + 6 orphans'.
            if strategy in ('row_per_entity', 'composite_key') and anchor:
                primary_anchor = anchor[0] if isinstance(anchor, list) else anchor
                target_anchor_count = _count_target_col_non_empty(
                    target_schema, primary_anchor
                )
                if target_anchor_count < 2:
                    overlap_anchor = _detect_entity_overlap_anchor(
                        target_schema, source_schema, exact_mappings
                    )
                    if overlap_anchor is None:
                        print(
                            f"Fast-path downgrade {strategy!r} → 'append' "
                            f"(target column {primary_anchor!r} has "
                            f"{target_anchor_count} non-empty value(s), "
                            f"no overlap on any mapped column)"
                        )
                        strategy, anchor, src_anchor, anchor_type = (
                            'append', None, None, None
                        )
                        reasoning += (
                            ' [local-downgrade: target effectively empty '
                            'for chosen anchor; routing through append]'
                        )
        ...
        # Fix 3: include per-column source uniqueness + coverage so the
        # LLM has the same hard signal the deterministic fast-path uses.
        # Without this the model can only see 2 sample values and a type
        # label, which is not enough to distinguish a real entity ID
        # (uniqueness ~1.0, e.g. SKU) from a noisy categorical column
        # (uniqueness < 0.5, e.g. Product Name with Widget A/B/C
        # repeating). With this, a fixture where the AI path is taken
        # (because some source column was unmatched) gets the same
        # safeguard the all-matched fast-path now has.
        source_summary = {}
        for col in unmatched_sources:
            uniq, cov = _compute_source_uniqueness(source_schema, col)
            source_summary[col] = {
                'type': source_schema['col_types'].get(col, 'unknown'),
                'samples': source_schema['col_samples'].get(col, [])[:2],
                'source_uniqueness': round(uniq, 2),
...
SOURCE-UNIQUENESS SIGNAL (use this when picking anchor_column):
- Each source column carries `source_uniqueness` (unique non-empty values / total non-empty values) and `source_coverage` (non-empty values / total rows).
- A real entity identifier has source_uniqueness ≥ 0.85 (e.g. SKU 12 unique / 13 rows = 0.92, Order ID 1.0).
- A categorical column has source_uniqueness < 0.5 (e.g. Product Name with 3 distinct values repeated across rows = 0.25). DO NOT pick a categorical column as anchor_column even if its header contains 'name'/'id'/'code'.
- Prefer the highest-uniqueness mapped column whose header suggests an identifier; fall back to a primary date column if no identifier qualifies; only choose "append" when neither a unique identifier nor a primary date is present in the source.
...
# Anchor-selection thresholds (Stage-1 fix, 2026-05).
#
# _FAST_PATH_UNIQ_THRESHOLD: minimum source-uniqueness ratio required for a
#   column matching _ENTITY_ID_HINTS to win the fast-path entity-ID scan.
#   Set to 0.85 so a single duplicate in a 13-row SKU column (12 unique /
#   13 = 0.923) still passes, while a categorical Product Name with 3
#   distinct values across 6 rows (= 0.5) is correctly rejected.
#
# _ENTITY_OVERLAP_UNIQ_THRESHOLD: minimum source-uniqueness required for
#   _detect_entity_overlap_anchor to promote 'append' -> 'row_per_entity'.
#   Stricter (0.9) than the fast-path because this override silently
#   changes write semantics; we only want it firing on near-perfect keys.
_FAST_PATH_UNIQ_THRESHOLD = 0.85
_ENTITY_OVERLAP_UNIQ_THRESHOLD = 0.9


def _compute_source_uniqueness(source_schema, source_col):
    """Return (uniqueness_ratio, coverage_ratio) for a source column.

    uniqueness_ratio = unique non-empty values / total non-empty values
    coverage_ratio   = total non-empty values / total raw rows

    Both default to 0.0 when the column has no data or is missing from
    raw_rows. Used by the fast-path anchor scorer
    (``_infer_strategy_local``), the entity-overlap promoter
    (``_detect_entity_overlap_anchor``), and the AI prompt augmentation
    so the model can distinguish a real entity identifier (uniqueness
    >= 0.85, e.g. SKU 12/13 = 0.923) from a noisy categorical column
    (uniqueness < 0.5, e.g. Product Name with Widget A/B/C repeating
    across rows). source_schema['raw_rows'] is a list of dicts (built
    by ``structure_source_data``) keyed by source header name.
    """
    raw_rows = source_schema.get('raw_rows') or []
    header_index = source_schema.get('header_index') or {}
    if not raw_rows:
        return 0.0, 0.0
    ...
    coverage = len(vals) / len(raw_rows)
    return uniqueness, coverage


def _count_target_col_non_empty(target_schema, tgt_col):
    """Return the number of non-empty values in ``tgt_col`` of the target.

    Used by the fast-path empty-target downgrade so that a fixture whose
    target is essentially empty for the chosen anchor (e.g. APPEND tab
    cleared, or a fresh tab whose only headers exist) does NOT route to
    row_per_entity-with-zero-matches; it falls through to 'append' which
    the multi-sheet aggregate-append branch handles correctly.
    """
    ...
    count = 0
    for r in data_rows:
        v = r[idx] if isinstance(r, list) and idx < len(r) else (
            r.get(tgt_col) if isinstance(r, dict) else None
        )
        if v is not None and str(v).strip():
            count += 1
    return count
...
def _detect_entity_overlap_anchor(target_schema, source_schema, mappings,
    ...
    for src_col, tgt_col in mappings.items():
        ...
        # Fix 2: source-uniqueness gate. Without this, a categorical
        # column like Product Name (Widget A repeats across multiple
        # rows) trivially overlaps the same Product Name in the target
        # and gets auto-promoted from 'append' to 'row_per_entity', even
        # though it is NOT a real entity identifier. The downstream
        # writer then runs intra-section duplicate detection and surfaces
        # a false "Widget A: pick a row" modal that the user cannot
        # answer sensibly. Skipping low-uniqueness candidates here keeps
        # the override conservative — we only auto-promote on near-perfect
        # keys (default 0.9), which match the SKU/Order-ID family.
        src_uniq, _src_cov = _compute_source_uniqueness(source_schema, src_col)
        if src_uniq < _ENTITY_OVERLAP_UNIQ_THRESHOLD:
            continue
...
def _infer_strategy_local(target_schema, source_schema, mappings):
    ...
    Anchor preference order (revised 2026-05 — Stage-1 fix):
        1. multi_section target            (writes per detected section)
        2. key_value 2-col label/value     (cheaper than full row analysis)
        3. entity ID columns SCORED by source uniqueness, threshold 0.85
           (sku/id/code/key/name) — picking the FIRST name-matching column
           caused fixtures where Product Name (3 distinct values across
           12 rows = 0.25 uniqueness) was selected over SKU (12 distinct /
           13 = 0.923). The downstream writer then either overwrote
           unrelated rows or surfaced false "Widget A: pick a row"
           intra-section duplicate modals. Scoring + threshold rejects
           noisy categorical columns even when they happen to match a
           keyword.
    ...
    # Entity-ID scan WITH source-uniqueness scoring (Fix 1, 2026-05).
    # Score every candidate whose target name matches an _ENTITY_ID_HINT
    # by source uniqueness (unique_non_empty / total_non_empty). Sort
    # high-to-low and pick the best candidate IFF its score clears
    # _FAST_PATH_UNIQ_THRESHOLD. Tie-break by source coverage so a fully-
    # populated SKU column wins over a Product Name that happens to be
    # 100% unique only because most of its rows are blank (TC-A02 case:
    # SKU 7/7=1.0 with coverage 7/7 vs Product Name 3/3=1.0 with
    # coverage 3/7 -> SKU still wins).
    entity_candidates = []
    for src, tgt in mappings.items():
        name_lower = tgt.lower() if tgt else ''
        if any(kw in name_lower for kw in _ENTITY_ID_HINTS):
            uniq, cov = _compute_source_uniqueness(source_schema, src)
            entity_candidates.append((uniq, cov, src, tgt))
    if entity_candidates:
        entity_candidates.sort(key=lambda x: (-x[0], -x[1], x[3]))
        best_uniq, best_cov, best_src, best_tgt = entity_candidates[0]
        if best_uniq >= _FAST_PATH_UNIQ_THRESHOLD:
            composite = _detect_composite_anchor(
                target_schema, mappings, best_tgt
            )
            if composite:
                disambig_tgt, disambig_src = composite
                return (
                    'composite_key',
                    [best_tgt, disambig_tgt],
                    [best_src, disambig_src],
                    'composite',
                )
            print(
                f"   _infer_strategy_local: entity-ID anchor {best_tgt!r} "
                f"(source_uniqueness={best_uniq:.2f}, coverage={best_cov:.2f}); "
                f"considered: "
                f"{[(t, round(u, 2), round(c, 2)) for u, c, _, t in entity_candidates]}"
            )
            return 'row_per_entity', best_tgt, best_src, 'id'
        # All entity-ID candidates are below threshold (e.g. Product Name
        # 3/6 = 0.5). Treat the entity scan as empty and fall through to
        # the date scan / append. This is the bit that keeps
        # Single-tab-append + 2-tab-append-only routing to 'append'
        # instead of pretending Product Name is a real key.
        print(
            f"   _infer_strategy_local: rejected entity-ID candidates "
            f"(below {_FAST_PATH_UNIQ_THRESHOLD:.2f} source uniqueness): "
            f"{[(t, round(u, 2)) for u, _, _, t in entity_candidates]}"
        )
