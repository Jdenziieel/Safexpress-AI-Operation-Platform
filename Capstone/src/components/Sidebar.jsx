...
const navItems = [
  ...
  { label: "Audit Logs", icon: ScrollText, path: "/audit-logs", allowedRoles: ["admin", "manager"] },
  { label: "Accounts", icon: Users, path: "/accounts", allowedRoles: ["admin"] },
  { label: "Activity Logs", icon: Activity, path: "/admin-activity-logs", allowedRoles: ["admin"] },
    ...
    const loadUserInfo = () => {
      try {
        ...
        if (userData && Object.keys(userData).length > 0) {
          ...
          console.log("Loaded user info:", { name: displayName, email: userEmail, picture: userPicture, isAdmin: adminStatus, role: getUserRole() });
        }
      } catch (error) {
