...
agent_capabilities = {
    ...
    "sheets_agent": {
        ...
        "tools": {
            ...
            "mirror_tabs": {
                "description": "Copy ALL TABS (or a filtered subset, or an explicit Source→Target rename map) from a SOURCE spreadsheet to a TARGET spreadsheet in a SINGLE compound step. Use this for 'copy all tabs', 'mirror', 'replicate', 'sync', 'duplicate every tab', and 'put Source.A into Target.B' requests where the user wants tab-level data movement between two spreadsheets. The per-tab loop runs INSIDE the sub-agent, so the planner does not need to know how many tabs the source has at plan time — that's the architectural reason this tool exists (Rule 20). Default behavior matches the most common user intent: by SAME-NAME matching (case-insensitive), create missing target tabs, clear any existing data in matched target tabs, then write the source tab values verbatim. Per-tab failures are non-fatal — one bad tab does not lose progress on the others. NEVER compose this from primitives (read_sheet → add_sheet_tab → clear_sheet → update_sheet) — the static plan format cannot express the dynamic 'for each source tab' loop, and the planner ends up generating useless transform_text meta-plans. DOES NOT APPLY to the delivery-order pipeline (Rule 16) — that uses validate_delivery_sheet + write_delivery_order_data with a fixed schema; mirror_tabs is for arbitrary sheet-to-sheet copies between two existing spreadsheets.",
                "args": {
                    "source_sheet_id": "str (required) — Google Sheets ID or URL of the SOURCE spreadsheet (the one being copied FROM). URLs are auto-parsed.",
                    "target_sheet_id": "str (required) — Google Sheets ID or URL of the TARGET spreadsheet (the one being copied TO). MUST differ from source_sheet_id — mirror_tabs cannot mirror a spreadsheet onto itself.",
                    "create_missing": "bool (optional, default True) — when True, tabs that exist in source but not in target are added to target. When False, source tabs without a matching target tab are skipped with status='skipped_missing' (use for 'only mirror tabs that already exist on both sides' / 'ignore non-matched tabs' / 'don't add new tabs' phrasings).",
                    "clear_existing": "bool (optional, default True) — when True, target tabs that already match (or are mapped to) a source tab are cleared (values only — formatting preserved) before the source data is written. When False with copy_data=True, the source data merges over existing target data cell-for-cell; cells outside the source range are NOT touched. Most users want True.",
                    "copy_data": "bool (optional, default True) — when True, source tab values are written to the target. When False, only tab structure is mirrored (rare; useful for setting up an empty target with the right tab layout).",
                    "include_tabs": "List[str] (optional) — whitelist filter on SOURCE tab names. Only tabs whose name matches (case-insensitive) are mirrored. Default None means all source tabs are mirrored. IGNORED when tab_mapping is provided (mapping doubles as the whitelist).",
                    "exclude_tabs": "List[str] (optional) — blacklist filter applied AFTER include_tabs. Tabs whose name matches (case-insensitive) are skipped. Useful for 'copy everything except the Notes tab' patterns. IGNORED when tab_mapping is provided.",
                    "tab_mapping": "Dict[str, str] (optional) — explicit {source_tab_name: target_tab_name} map for the 'put Source.A into Target.B' rename case. When provided, the mapping doubles as a whitelist — ONLY mappings in this dict are processed and unmapped source tabs are NOT mirrored even if their names match target tabs. Source-side keys are case-insensitive (so 'food' matches a real tab titled 'Food'); target-side values are used verbatim (preserves the user's preferred case for the destination tab). When the source tab named in a mapping does not exist on the source, the entry is recorded as status='skipped_source_missing' so the user sees the typo. Default None means same-name matching with optional include_tabs/exclude_tabs filtering.",
                },
                "returns": ["success", "source_sheet_id", "target_sheet_id", "source_title", "target_title", "tabs_processed", "tabs_total", "tabs_succeeded", "tabs_failed", "tabs_skipped", "tabs_created", "tabs_cleared", "rows_total", "warnings", "message", "error", "error_type"],
                "returns_detail": "tabs_processed is a list of per-tab dicts with keys {tab_name (source), target_tab_name (destination — equals tab_name unless tab_mapping renamed it), created, cleared, rows_copied, columns_copied, status, error}. status is one of 'success', 'error', 'skipped_missing' (target tab missing AND create_missing=False), or 'skipped_source_missing' (tab_mapping referenced a source tab that does not exist). tabs_skipped is the count of tabs with skipped_* status. Overall success=True iff tabs_failed==0 AND tabs_skipped_typo (skipped_source_missing) count is 0 — pure intentional skips with create_missing=False are still considered successful (the user explicitly opted out). error_type='partial_failure' when tabs failed mid-run; 'bad_input' when only typo'd source tab names caused the failure.",
                "can_be_derived_from": {
                    "source_sheet_id": "drive_agent.search_files",
                    "target_sheet_id": "drive_agent.search_files",
                },
            },
            "get_sheet_headers": {
