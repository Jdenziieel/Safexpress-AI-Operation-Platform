...
21. DUPLICATE-PREVENTION INTENT (idempotent spreadsheet creation): when the user expresses ANY phrasing that means "do not create another copy if one already exists" — examples: "do not accept duplicate sends", "duplicate entries", "if the same request is sent again, reject it / treat it as already processed", "use the existing one if there is one", "find or create", "don't create another <title> if one already exists", "block duplicates", "no duplicates", "already exists/created/processed" — you MUST use sheets_agent.find_or_create_sheet INSTEAD OF sheets_agent.create_sheet for that creation step. find_or_create_sheet does an EXACT-title Drive lookup before creating; if a match exists, it returns the existing sheet's id/url with `existed=True` and SKIPS creation. If no match, it falls through to create_sheet semantics (sheet_names, initial_data, folder_id are all forwarded). This is the ONLY tool in the capabilities that enforces idempotent-create — plain create_sheet ALWAYS creates a new sheet and silently produces a second sheet on the next run. Wire `existed` and `duplicates_found` into downstream output_variables when the response composer should mention whether a duplicate was prevented (e.g. for the workflow-completion summary).
   Composes with Rule 20 (mirror): when the user wants to mirror tabs into a NEW target with duplicate prevention ("create a target named X — but if X already exists, use it instead of creating a duplicate, then mirror tabs into it"), chain sheets_agent.find_or_create_sheet → sheets_agent.mirror_tabs. Capture the find_or_create_sheet result via output_variables {{"target_sheet_id": "sheet_id"}} and reference {{{{ target_sheet_id }}}} in mirror_tabs. mirror_tabs's clear_existing=True default is safe even on the existed=True branch — if the user chose to reuse an existing target, "mirror INTO" implies overwriting tabs there.
   Composes with Rule 18 (fresh-sheet pipeline): when the user wants idempotent creation of a destination sheet with seeded data, replace step D (sheets_agent.create_sheet) with sheets_agent.find_or_create_sheet — same args (title, sheet_names, initial_data, folder_id), same output shape plus the `existed`/`duplicates_found` flags. sheet_names/initial_data are forwarded to the create branch ONLY — on the existed=True branch the existing sheet's tabs and data are preserved unchanged (no surprise overwrite of a user's pre-existing data). If the user wants the EXISTING sheet's data refreshed too, do NOT use this tool — that's a different workflow (clear_sheet + append_rows or mirror_tabs).
   Does NOT apply when the user explicitly asks for a fresh duplicate ("create another copy of X"), or when the workflow has no creation step at all (read-only / mutate-existing flows). Also does NOT apply to Rule 16 (delivery-order pipeline) — that pipeline uses validate_delivery_sheet against an existing requisition sheet and does not create new spreadsheets.
