...
function Accounts() {
  ...
  // Fetch accounts from the database (AWS Lambda endpoint - no trailing slash)
    ...
    if (location.state?.updatedAccount) {
      ...
      setAccounts(prevAccounts =>
        prevAccounts.map(account =>
          account.gmail === updatedAccount.gmail ? { ...account, ...updatedAccount } : account
        )
      );
  ...
  const handleDeactivate = async (email, accountName) => {
    const result = await Swal.fire({
      icon: 'warning',
      title: 'Deactivate Account?',
      html: `Are you sure you want to deactivate this account?<br><br><strong>${accountName}</strong>`,
    ...
    if (result.isConfirmed) {
      try {
        // Use PATCH to update is_active (same as activate)
        await api.patch(`/api/users/${email}`, { is_active: false });
        setAccounts(prevAccounts =>
          prevAccounts.map(account =>
            account.gmail === email
  ...
  const handleActivate = async (email, accountName) => {
    const result = await Swal.fire({
      icon: 'info',
      title: 'Activate Account?',
      html: `Are you sure you want to activate this account?<br><br><strong>${accountName}</strong>`,
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        // AWS Lambda endpoint - use gmail as path parameter
        await api.patch(`/api/users/${email}`, { is_active: true });
        setAccounts(prevAccounts =>
          prevAccounts.map(account =>
            account.gmail === email
        ...
        await Swal.fire({
          icon: 'error',
          title: 'Error!',
          text: error.response?.data?.error || 'Failed to activate account.',
          confirmButtonColor: '#26326e'
        });
      }
    }
  ...
  const handleViewAccountLogs = (email) => {
    navigate(`/admin-activity-logs?target_email=${encodeURIComponent(email)}&scope=account`);
  };
                      ...
                      <td style={{ padding: '16px 16px', textAlign: 'center' }}>
                        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10 }}>
                          ...
                          <button 
                            ...
                            onClick={() => handleViewAccountLogs(account.gmail)}
                          >
                          ...
                          {showDeactivated ? (
                            <button 
                              ...
                              aria-label="Activate"
                              title="Activate account"
                              onClick={() => handleActivate(account.gmail, account.fullname)}
                            >
                              <UserCheck size={18} />
                            </button>
                          ) : (
                            <button 
                              ...
                              title="Deactivate account"
                              onClick={() => handleDeactivate(account.gmail, account.fullname)}
                            >
                              <UserX size={18} />
                            </button>
                          )}
