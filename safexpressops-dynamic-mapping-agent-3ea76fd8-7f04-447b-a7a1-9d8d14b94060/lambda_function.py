...
def run_dynamic_mapping(inputs):
    ...
    if (
        ...
        return _run_multi_sheet_column_merge(
            ...
            # Stage 2 anchor authority. Preview stashes the planner's
            # actual target anchor name here; the run path re-applies
            # the override when cached_per_sheet is missing or stale.
            # Defensive — a fresh confirm without the cache would
            # otherwise fall back to per-sheet find_identifier picks
            # and silently regress the bug.
            planner_anchor=strategy_metadata.get('planner_anchor'),
        )
            ...
            if not target_sections_for_aggregate or len(target_sections_for_aggregate) <= 1:
                ...
                if column_merge_plan:
                    ...
                    return _preview_multi_sheet_column_merge(
                        ...
                        conflict_choices=inputs.get('conflict_choices'),
                        planner_anchor=column_merge_plan.get('shared_anchor_target_col'),
                    )
...
def _detect_sections_local(raw_values, min_section_rows=0):
    ...
    # If exactly one explicit titled section was found, probe rows above it for
    # an untitled top block where the first non-empty row is a plausible header.
...
def _detect_sections_single_pass(raw_values, min_section_rows=0):
    """Variant of :func:`_detect_sections_local` that returns 1+ sections
    without the ``>= 2 sections`` suppression. KEEP IN LOCK-STEP with
    _detect_sections_local's inner loop body AND with
    ``mapping_agent_api._detect_sections_single_pass`` (the latter is the
    canonical version — it additionally supports PATH 2 "headerless"
    sections used by the source-side cross-tab × per-section routing).

    Used by :func:`_write_multi_section` so single-titled-section targets
    (e.g. ``Inbound Matrix`` with row 1 = "Inbound Metrics" title and
    row 2 = "Date|Trucks|Pallets" headers) are accepted on the write
    path. Without this fallback the write fails with "No sections
    detected in target sheet" even though the preview side already
    routed correctly via :func:`mapping_agent_api._detect_sections_single_pass`.
...
def _plan_multi_sheet_column_merge(sheets_list, target_headers):
    ...
    # Map normalized header -> first actual target header that produced it.
    # Stage 2 fix needs the actual (case-preserving) target column name to
    # pass through to the preview/run override so we don't have to re-resolve
    # the case on every sheet's column_mappings.
    target_norm_to_actual = {}
    for h in (target_headers or []):
        if h:
            norm = _norm_header(h)
            if norm and norm not in target_norm_to_actual:
                target_norm_to_actual[norm] = h
    target_set = set(target_norm_to_actual.keys())
    if not target_set:
    ...
    return {
        ...
        # Actual target header (case-preserved) that the normalized
        # ``chosen_anchor`` resolves to. Consumed by
        # _preview_multi_sheet_column_merge / _run_multi_sheet_column_merge
        # via the Stage 2 anchor-authority override so per-sheet
        # find_identifier picks can't quietly re-key the writer onto a
        # different column than the one the planner chose.
        'shared_anchor_target_col': target_norm_to_actual.get(chosen_anchor),
...
def _override_anchor_with_planner(per_sheet_state, planner_anchor):
    """Override a column-merge per-sheet anchor with the planner's choice.

    Stage 2 fix (2026-05). The column-merge planner picks a SHARED anchor
    by looking at every chosen sheet at once (canonical case: ``DATE``,
    the only header all sheets contribute). Each sheet's local
    ``find_identifier`` runs in isolation though, sees only its own
    schema, and tends to pick a per-row identifier (``SKU`` /
    ``Product Name``) instead of the shared key. When those local picks
    win, the writer keys rows by ``SKU`` and the unmapped (but
    target-existing) ``DATE`` cells get overwritten with raw epoch ms
    from the source — the "dates are all messed up" bug.

    This helper rewrites a per-sheet state in place to use ``planner_anchor``
    (an actual target header name, NOT the normalized form). Returns
    True only when the sheet has a column mapping that lands on
    ``planner_anchor`` — caller MUST refuse to apply when ANY sheet would
    fail this check, otherwise base_strategy/base_anchor would drift
    across sheets and the strategy_drift gate downstream would bail
    the whole preview.

    Side effects on success:
      * ``anchor_column``       -> planner_anchor
      * ``source_anchor_names`` -> [<src col mapped to planner_anchor>]
      * ``write_strategy``      -> 'row_per_date' if date-like, else 'row_per_entity'
      * ``is_date_anchor``      -> True iff planner_anchor name contains 'date'
      * ``identification`` dict (when present) is mirrored so the
        per_sheet_cache the FE round-trips back at confirm time carries
        the overridden decision verbatim — the run path replays it
        without re-deriving.
    """
    cm = per_sheet_state.get('column_mappings') or {}
    src_col = None
    for s, t in cm.items():
        if t == planner_anchor:
            src_col = s
    ...
    is_date = 'date' in str(planner_anchor).lower()
    new_strategy = 'row_per_date' if is_date else 'row_per_entity'
    new_anchor_type = 'date' if is_date else 'id'

    per_sheet_state['anchor_column']       = planner_anchor
    per_sheet_state['source_anchor_names'] = [src_col]
    per_sheet_state['write_strategy']      = new_strategy
    per_sheet_state['is_date_anchor']      = is_date

    ident = per_sheet_state.get('identification')
    if isinstance(ident, dict):
        ident['anchor_column']  = planner_anchor
        ident['source_anchor']  = src_col
        ident['write_strategy'] = new_strategy
        ident['anchor_type']    = new_anchor_type
...
def _preview_multi_sheet_column_merge(
    ...
    # Stage 2 anchor authority (2026-05). The column-merge planner already
    # picked a SHARED anchor (typically DATE) by looking at every chosen
    # sheet at once — but each sheet's local find_identifier picks a
    # per-row identifier (SKU / Product Name) in isolation. Without
    # honoring the planner's choice, the writer keys rows by SKU and the
    # unmapped DATE cells get overwritten with raw epoch ms from the
    # source ("dates are all messed up" bug). We apply the override only
    # when EVERY chosen sheet has a column mapping that lands on the
    # planner's anchor; otherwise some sheets would still key on the
    # find_identifier pick and the strategy_drift gate below would
    # correctly bail. Logged either way so CloudWatch shows when the
    # override fires vs falls through.
    if planner_anchor:
        eligible_sheets = []
        ineligible_sheets = []
        for st in per_sheet_states:
            cm_vals = (st.get('column_mappings') or {}).values()
            if any(t == planner_anchor for t in cm_vals):
                eligible_sheets.append(st['sheet_name'])
            else:
                ineligible_sheets.append(st['sheet_name'])
        if not ineligible_sheets:
            for st in per_sheet_states:
                _override_anchor_with_planner(st, planner_anchor)
            print(
                f"   column-merge anchor authority: overrode per-sheet "
                f"anchors with planner_anchor={planner_anchor!r} on "
                f"{len(eligible_sheets)} sheet(s) {eligible_sheets}"
            )
        else:
            print(
                f"   column-merge anchor authority: skipped — "
                f"planner_anchor={planner_anchor!r} not present in "
                f"column_mappings of {ineligible_sheets}; keeping "
                f"find_identifier picks (eligible={eligible_sheets})"
            )
    ...
    if all_intra_conflicts:
        ...
        return {
            ...
            'strategy_metadata': {
                ...
                'underlying_strategy':   base_strategy,
                'anchor_columns':        base_anchor if isinstance(base_anchor, list) else None,
                # Stage 2: round-trip the planner's anchor so the next
                # preview/confirm re-applies the override even if it
                # arrives without per_sheet_cache (e.g. older FE bundle
                # or unexpected cache invalidation).
    ...
    if unresolved_count > 0:
        ...
        return {
            ...
            'strategy_metadata': {
                ...
                'underlying_strategy':   base_strategy,
                'anchor_columns':        base_anchor if isinstance(base_anchor, list) else None,
                # Stage 2: same round-trip guarantee as the intra-section
                # branch — keeps the planner's anchor on the FE's
                # strategy_metadata so the next preview/run re-applies
                # the override.
                'planner_anchor':        planner_anchor,
    ...
    final_strategy_metadata = {
        ...
        'intra_section_choices': raw_intra_choices,
        # Stage 2: round-trip the planner's anchor so the run path can
        # re-apply the anchor-authority override on confirm even when
        # per_sheet_cache is absent / stale.
        'planner_anchor':       planner_anchor,
...
def _run_multi_sheet_column_merge(
    ...
    # Stage 2 anchor authority (run-side mirror of preview).
    # When cached_per_sheet was present the per-sheet states already
    # carry the overridden anchor — apply is a no-op (the column_mapping
    # check below confirms eligibility but the values match). When the
    # cache was missing (older FE bundle, or a confirm that lost
    # strategy_metadata in transit), per-sheet find_identifier picks
    # would otherwise leak SKU back as the anchor and the writer would
    # again overwrite DATE with epoch ms. The orchestrator passes the
    # planner_anchor we stashed in strategy_metadata at preview time
    # so this defensive re-application stays cheap and correct.
    if planner_anchor:
        eligible_sheets = []
        ineligible_sheets = []
        for st in per_sheet_states:
        ...
        if not ineligible_sheets:
            for st in per_sheet_states:
                _override_anchor_with_planner(st, planner_anchor)
            print(
                f"   column-merge run anchor authority: applied "
                f"planner_anchor={planner_anchor!r} on {len(eligible_sheets)} "
                f"sheet(s) {eligible_sheets}"
            )
        else:
            print(
                f"   column-merge run anchor authority: skipped — "
                f"planner_anchor={planner_anchor!r} not in column_mappings "
                f"of {ineligible_sheets}; using cached/find_identifier picks"
            )
...
def _write_multi_section(anchor_column, transformed, sheet_id, sheet_name,
    ...
    if not sections:
        # Fallback for single-titled-section targets (e.g. Inbound Matrix:
        # row 1 = "Inbound Metrics" title, row 2 = "Date|Trucks|Pallets"
        # headers, then data). _detect_sections_local suppresses results
        # below 2 sections so a sheet with exactly one titled section
        # falls through here. The preview path already accepts this shape
        # via mapping_agent_api._detect_sections_single_pass + the
        # structure_target_data single-titled fallback, so the writer
        # must accept it too or confirm fails with "No sections detected"
        # even though the diff preview reported real fills.
        single_pass = _detect_sections_single_pass(raw_values)
        titled_single = [s for s in single_pass if s.get('title')]
        if len(titled_single) == 1:
            sections = titled_single
            print(
                f"   Multi-section: accepted single titled section "
                f"'{titled_single[0].get('title')}' via single-pass fallback"
            )
        else:
