"""supervisor-admin-budget-get — GET /admin/settings/budget"""
import os
import sys
...
def lambda_handler(event, context):
    ...
    with set_request_context_lambda(event):
        try:
            ...
            # Fire the budget guardrail email if we just crossed the
            # warning or critical threshold (deduped to once per
            # threshold per calendar month). Failure here MUST NOT
            # break the dashboard refresh — maybe_send_budget_alert
            # swallows SES errors and prints to CloudWatch.
            try:
                maybe_send_budget_alert(storage, snapshot)
            except Exception as alert_err:
                print(f"[budget-get] alert dispatch raised (non-fatal): {alert_err}")
            return success_response(snapshot)
        except Exception as e:
            return error_response(500, f"Error retrieving budget: {e}")
