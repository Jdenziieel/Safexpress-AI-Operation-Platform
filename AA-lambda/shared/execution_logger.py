...
# In AWS Lambda, /var/task (where this module lives) is read-only and the
# only writable spot is /tmp. AWS sets ``AWS_LAMBDA_FUNCTION_NAME`` for us
# so we can detect the runtime cheaply. CloudWatch already captures every
# print/log statement, so the on-disk trace.log is a developer convenience
# only — we still write it under /tmp in case someone exec's into the
# container for debugging, but losing it is not a failure mode.
...
try:
    ...
    # Defense-in-depth: even on /tmp this can fail (full disk, permissions).
    # Falls through; the per-write try/except below will silently no-op.
    pass
...
class ExecutionTracer:
    ...
    def _write(self, level: str, category: str, message: str, data: dict = None):
        ...
        # In Lambda, /var/task is read-only and even /tmp can fail (full
        # disk, ENOSPC). CloudWatch captures stdout already, so trace.log
        # is best-effort — silently swallow filesystem errors instead of
        # crashing the whole request mid-flight.
        with self._lock:
