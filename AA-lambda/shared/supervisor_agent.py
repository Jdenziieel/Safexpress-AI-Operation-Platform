...
""" + capability_summary
    ...
    # Save the plan to a file for inspection. Best-effort only — under
    # Lambda OUTPUT_DIR resolves to /tmp/agent_outputs (writable), but even
    # /tmp can fail (ENOSPC, permissions, container quirks). The plan
    # already lives in the in-memory state and is logged via trace, so a
    # filesystem write failure must NEVER crash the workflow.
    ...
    try:
        ...
        print(f"\n[supervisor_agent] plan dump skipped (non-fatal): {e}")
    print("=" * 60 + "\n")
