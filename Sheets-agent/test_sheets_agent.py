...
def test_create_sheet_dry_run():
    ...
    if not result.get("success"):
        ...
        if "Authentication" in error or "Credentials" in error:
            print("✅ API structure correct (authentication failed as expected with test credentials)")
            print(f"   Error: {error}")
        else:
...
def test_upload_mapped_data_structure():
    ...
    transformed_data = json.dumps([
        {"Present": "John Doe", "Total Manhours": "40", "Losttime Incident": "0", "Date": "2025-01-28"},
        {"Present": "Jane Smith", "Total Manhours": "35", "Losttime Incident": "1", "Date": "2025-01-28"}
    ])
...
Jane,35"""
            ...
            parse_result = requests.post("http://localhost:8004/execute", json={
                "tool": "parse_file",
            ...
            if parse_result.get("success"):
                ...
                # Step 2: Map
                map_result = requests.post("http://localhost:8004/execute", json={
                ...
                if map_result.get("success"):
                    ...
                    # Step 3: Transform
                    transform_result = requests.post("http://localhost:8004/execute", json={
                        "tool": "transform_data",
                        "inputs": {
                    ...
                    if transform_result.get("success"):
                        ...
                        upload_result = requests.post(f"{BASE_URL}/execute", json={
                            "tool": "upload_mapped_data",
                            "inputs": {
                                "sheet_id": "test_id",
                                "transformed_data": transformed_data,
