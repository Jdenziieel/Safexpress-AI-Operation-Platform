...
def error_response(message: str, status_code: int = 400, error_code: str = None, details: dict = None) -> Dict:
    """
    ...
    # Also include 'detail' structure for frontend compatibility
    # Frontend expects: error.response?.data?.detail?.error and detail?.user_message
    body['detail'] = {
        'error': error_code.lower() if error_code else 'error',
