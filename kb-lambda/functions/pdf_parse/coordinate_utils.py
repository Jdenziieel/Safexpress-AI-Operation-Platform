...
def calculate_encompassing_box(tables):
    """
    Merge bounding boxes from one or more table elements.
    Returns a single box dict when all tables are on one page,
    or a list of per-page box dicts (each with a 'page' key) when they span
    multiple pages -- matching the format that calculate_chunk_box produces
    for cross-page text content.
    """
    if not tables:
        return {"l": 0, "t": 0, "r": 0, "b": 0}
    ...
    if len(boxes_by_page) == 1:
        page = next(iter(boxes_by_page))
        bxs = boxes_by_page[page]
        return {
            "l": min(b.get("l", 0) for b in bxs),
            "t": min(b.get("t", 0) for b in bxs),
            "r": max(b.get("r", 0) for b in bxs),
            "b": max(b.get("b", 0) for b in bxs),
        }
    ...
    for element in structured:
        if element.get("type") == "text" and "text" in element:
            ...
            if "page_height" in element:
                line["page_height"] = element["page_height"]
            if "page_width" in element:
                line["page_width"] = element["page_width"]
            lines.append(line)
    return lines
